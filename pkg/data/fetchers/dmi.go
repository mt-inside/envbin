...
func getDmiFirmware(ctx context.Context, log logr.Logger, vals chan<- trie.InsertMsg) {
	...
	vals <- trie.Insert(trie.Some(u8(data[0].SystemBIOSMajorRelease)), "Hardware", "Firmware", "Major")
	vals <- trie.Insert(trie.Some(u8(data[0].SystemBIOSMinorRelease)), "Hardware", "Firmware", "Minor")
}
...
func getDmiMotherboard(ctx context.Context, log logr.Logger, vals chan<- trie.InsertMsg) {
	...
	vals <- trie.Insert(trie.Some(data[0].ProductName), "Hardware", "Motherboard", "Product")
...
func getDmiSystem(ctx context.Context, log logr.Logger, vals chan<- trie.InsertMsg) {
	...
	vals <- trie.Insert(trie.Some(chassis[0].SKUNumber), "Hardware", "Chassis", "Product")
	...
	vals <- trie.Insert(trie.Some(system[0].ProductName), "Hardware", "System", "Product")
...
func getDmiRAM(ctx context.Context, log logr.Logger, vals chan<- trie.InsertMsg) {
	...
	for i, m := range data {
		...
		vals <- trie.Insert(trie.Some(m.PartNumber), "Hardware", "RAM", strconv.Itoa(i), "Product")
...
func getDmiCPU(ctx context.Context, log logr.Logger, vals chan<- trie.InsertMsg) {
	...
	vals <- trie.Insert(trie.Some(data[0].Version), "Hardware", "CPU", "Product")
